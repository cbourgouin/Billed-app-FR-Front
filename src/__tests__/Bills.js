...
describe('Given I am connected as an employee', () => {
  describe('When I am on Bills Page', () => {
    test('Then bill icon in vertical layout should be highlighted', async () => {
      ...
      const backgroundColor = window.getComputedStyle(windowIcon).backgroundColor;
      expect(backgroundColor).toBe('#7bb1f7')
    });
    ...
    test('Then fonction getBill return the same bills', async () => {
      ...
      const returnGetBill = await bill.getBills();
      const expectedRes = await mockStore.bills().list();
      expect(returnGetBill.length).toEqual(expectedRes.length);
    });
